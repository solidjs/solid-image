  ...
  return (
    <For each={PHOTOS}>
      {(photo, index) => {
        ...
        const bytes = placeholder && "url" in placeholder ? placeholder.url.length : 0;
        ...
        return (
          <figure>
            ...
            />
            <figcaption>
              {photo.alt}. The preview is {bytes} bytes of inline data.
            </figcaption>
          </figure>
        );
      }}
...
export default function App() {
  ...
  return (
    <main>
      <h1>LQIP preview</h1>
      <p>
        Each image comes with a 20px wide copy of itself, inlined in the page. The browser scales
        it up behind the image, so a blurred version shows until the real image loads.
      </p>
